 ...
 * Covers seed data integrity and pure business logic (extracted from hooks).
 */
import {
  SEED_PROFILES,
  SEED_USER_PROFILES,
...
describe("SEED_PROFILES", () => {
  ...
  test("expected profiles exist", () => {
    const ids = SEED_PROFILES.map((p) => p.id);
    expect(ids).toContain("profile_admin");
    expect(ids).toContain("profile_lo");
    expect(ids).toContain("profile_ca");
    expect(ids).toContain("profile_co");
    expect(ids).toContain("profile_ro");
  });

  test("System Administrator profile has name 'System Administrator'", () => {
    const admin = SEED_PROFILES.find((p) => p.id === "profile_admin");
    expect(admin?.name).toBe("System Administrator");
  });

  test("Read Only profile exists with correct name", () => {
...
describe("SEED_USER_PROFILES", () => {
  ...
  test("all profileIds reference known SEED_PROFILES", () => {
    ...
    SEED_USER_PROFILES.forEach((up) => {
      expect(knownIds.has(up.profileId)).toBe(true);
    });
  });

  test("A100001 is assigned to profile_admin", () => {
    const up = SEED_USER_PROFILES.find(
...
describe("Profile CRUD pure logic", () => {
  ...
  test("deleteProfile removes profile by id", () => {
  ...
  test("deleteProfile removes associated user-profiles", () => {
    const userProfiles: UserProfile[] = [
      { id: "up1", userSid: "U1", profileId: "p1", createdAt: "2024-01-01T00:00:00.000Z" },
      { id: "up2", userSid: "U2", profileId: "p2", createdAt: "2024-01-01T00:00:00.000Z" },
      { id: "up3", userSid: "U3", profileId: "p1", createdAt: "2024-01-01T00:00:00.000Z" },
    ];
    const result = userProfiles.filter((up) => up.profileId !== "p1");
    expect(result).toHaveLength(1);
    expect(result[0].profileId).toBe("p2");
  });

  test("updateProfile applies patch and preserves other fields", () => {
    const profiles = [makeProfile({ id: "p1", name: "Old Name" })];
    const updated = profiles.map((p) =>
      p.id === "p1" ? { ...p, name: "New Name" } : p
    );
    expect(updated[0].name).toBe("New Name");
    expect(updated[0].description).toBe("A test"); // preserved
    expect(updated[0].colorHex).toBe("#1B7F9E");   // preserved
  });

  test("getProfile returns correct profile", () => {
    const profiles = [makeProfile({ id: "p1" }), makeProfile({ id: "p2", name: "Other" })];
    const result = profiles.find((p) => p.id === "p2");
    expect(result?.name).toBe("Other");
  });

  test("getProfile returns undefined for missing id", () => {
...
describe("assignUserProfile / removeUserProfile logic", () => {
  const base: UserProfile[] = [
    { id: "up1", userSid: "U1", profileId: "admin", createdAt: "2024-01-01T00:00:00.000Z" },
  ];

  test("assignUserProfile adds new assignment when not already present", () => {
    const notAlreadyPresent = !base.some(
      (up) => up.userSid === "U2" && up.profileId === "lo"
    );
    expect(notAlreadyPresent).toBe(true);
    // Simulate the add
    const result = [...base, { id: "up2", userSid: "U2", profileId: "lo", createdAt: "" }];
    expect(result).toHaveLength(2);
  });

  test("assignUserProfile is idempotent (does not add duplicate)", () => {
    const alreadyPresent = base.some(
      (up) => up.userSid === "U1" && up.profileId === "admin"
    );
    // Simulate the guard: if already present, don't add
    const result = alreadyPresent ? base : [...base, { id: "up_new", userSid: "U1", profileId: "admin", createdAt: "" }];
    expect(result).toHaveLength(1); // still 1, not 2
  });

  test("removeUserProfile removes correct assignment by userSid + profileId", () => {
  ...
  test("removeUserProfile on non-existent entry leaves array unchanged", () => {
    const result = base.filter(
      (up) => !(up.userSid === "UNKNOWN" && up.profileId === "admin")
    );
