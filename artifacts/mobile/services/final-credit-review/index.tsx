...
const KEYS = {
  records: "svc_fcr_v1",
  conditions: "svc_fcr_conditions_v1",
  exceptions: "svc_fcr_exceptions_v1",
};
...
const [FinalCreditReviewServiceProvider, useFinalCreditReviewService] = createContextHook(() => {
  ...
  const updateFCR = useCallback(async (applicationId: string, patch: Partial<FCRRecord>) => {
    ...
    if (existing) {
      ...
      await persistRecords([...records, { applicationId, commitmentLetterRecommended: false,
        commitmentLetterIssuedDate: "", updatedAt: now(), ...patch }]);
    }
  ...
  const addCondition = useCallback(async (applicationId: string, data: Omit<Condition, "id" | "applicationId" | "createdAt" | "updatedAt">): Promise<Condition> => {
  ...
  const addException = useCallback(async (applicationId: string, data: Omit<Exception, "id" | "applicationId" | "createdAt" | "updatedAt">): Promise<Exception> => {
  ...
  }, [persistRecords, persistConditions, persistExceptions]);

  const clearData = useCallback(async () => {
    await Promise.all([persistRecords([]), persistConditions([]), persistExceptions([])]);
  }, [persistRecords, persistConditions, persistExceptions]);
  ...
  }, [records, conditions, exceptions, persistRecords, persistConditions, persistExceptions]);
